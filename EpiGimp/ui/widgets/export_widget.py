...
from PySide6.QtCore import Qt, QDir, QSize, QFileInfo
...
class ExportWidget(QDialog):
    ...
    def setup_size(self):
        """Set dialog size to fit screen with margins"""
    ...
    def setup_ui(self):
        ...
        # File list
        # Replace QListWidget with QTreeWidget for file details
        ...
        header.setSectionResizeMode(1, QHeaderView.ResizeToContents)  # Type fits content
        ...
        splitter.setSizes([400, 600])  # Adjusted for larger dialog
    ...
    def populate_folders(self):
        """Initialize the folder tree with expandable nodes"""
    ...
    def load_subfolders(self, parent_item):
        """Load subfolders for the given parent item"""
        ...
        try:
            ...
            # Add an item indicating no access
    ...
    def populate_files(self, folder_path):
        """Populate file list with Name, Type, and Size columns"""
        ...
        try:
            ...
            for filename in dir_obj.entryList():
                ...
                # Create tree item with three columns
                item = QTreeWidgetItem([name, file_type, size])
                item.setData(0, Qt.UserRole, file_path)
                self.file_list.addTopLevelItem(item)  # Fixed: changed from addItem()
                
        except PermissionError:
            # Add error message spanning all columns
    ...
    def on_file_double_clicked(self, item, column):
        ...
        filename = item.text(0)  # Get text from first column (Name)
    ...
    def export_image(self, image_data):
        """Show dialog and export if user confirms"""
